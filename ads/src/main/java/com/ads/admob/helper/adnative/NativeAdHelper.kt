...
class NativeAdHelper(
    ...
    init {
        ...
        adNativeState.onEach { logZ("adNativeState(${it::class.java.simpleName})") }
            .launchIn(lifecycleOwner.lifecycleScope)
    ...
    fun setShimmerLayoutView(shimmerLayoutView: ShimmerFrameLayout) = apply {
        kotlin.runCatching {
            ...
            if (nativeAd != null && adNativeState.value is AdNativeState.Loaded) {
    ...
    private fun createNativeAds(activity: Activity) {
        if (canRequestAds()) {
            if (config.idAdsPriority != null) {
                requestAdsAlternate(
                    activity, config.idAdsPriority, config.idAds, invokeListenerAdCallback()
                )
            } else {
    ...
    private fun requestAdsAlternate(
        ...
        AdmobFactory.INSTANCE.requestNativeAd(
                activity, idAdPriority, config.adPlacement, false, object : NativeAdCallback {
    ...
    /**
     * Detaches the currently bound content/shimmer views without cancelling the ad request.
     * Call this when the view that was showing this ad is no longer on screen (e.g. a shared
     * ad container got reassigned to another page), so a late-arriving [AdNativeState.Loaded]
     * doesn't paint this ad into a view that now belongs to someone else.
     */
    fun detachContentView() {
