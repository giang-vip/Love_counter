...
class NameTestRepositoryImpl(context: Context) : NameTestRepository {

    private val db = AppDatabase.getDatabase(context)
    private val nameTestResultDao = db.nameTestResultDao()
    ...
    override fun calculateNameTestResult(yourName: String, partnerName: String): NameTestResult {
        ...
        try {
            val entity = NameTestResultEntity(
                yourName = name1,
                partnerName = name2,
                percent = percent,
                description = description
            )
            nameTestResultDao.insertResult(entity)
        } catch (e: Exception) {
            e.printStackTrace()
        }
